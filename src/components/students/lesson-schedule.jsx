...
function LessonsSchedule({ studentId }) {
        ...
        <div className="grid grid-cols-8 gap-x-1 md:gap-x-1.5 min-w-[50rem] w-full">
          ...
          {times.map((time, timeIndex) => (
            <React.Fragment key={timeIndex}>
              <div className="sticky left-0 z-20 flex items-center justify-center gap-1 md:gap-2 font-semibold text-center dark:bg-background p-2 bg-background">
                <Clock className="w-4 h-4" />
                <span>{time}</span>
              </div>
              {days.map((day) => {
                const groupForDay = studentGroups.find((schedule) => {
